...
import { saveSettingsAction, SettingsFormState } from "@/app/admin/settings/actions";
...
export default function SettingsPage() {
  ...
  const formAction = async (formData: FormData) => {
    const result = await saveSettingsAction(state, formData);
    setState(result);
  };
  ...
  const handleLogoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        toast({ title: "Site Logo Updated", description: "Click 'Save All Settings' to apply the change." });
      };
  ...
  const handleFaviconChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      const reader = new FileReader();
      reader.onloadend = () => {
        const newFaviconUrl = reader.result as string;
        setFaviconUrl(newFaviconUrl);
        toast({ title: "Favicon Updated", description: "Your site's favicon has been changed live." });
      };
      reader.readAsDataURL(file);
      event.target.value = "";
    }
