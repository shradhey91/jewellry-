...
import { getUserByPhoneNumber, findOrCreateUserByPhone, getUserByEmail, verifyPassword, createUser } from '@/lib/server/auth';
...
// Mock OTP store - NOT FOR PRODUCTION
const otpStore: Record<string, { otp: string; expires: number }> = {};

const phoneSchema = z.string().regex(/^\d{10,15}$/, 'Please enter a valid phone number (10-15 digits, no symbols).');

async function sendOtp(phone: string, otp: string) {
  // In a real app, this would check which service (WhatsApp, SMS) is configured.
  console.log(`[MOCK OTP] Sending OTP ${otp} to ${phone} via configured service (SMS/WhatsApp).`);
...
export async function loginAction(prevState: LoginState, formData: FormData): Promise<LoginState> {
    ...
    const user = await getUserByEmail(identifier) || await getUserByPhoneNumber(identifier);
    ...
    const sessionPayload = { id: user.id, name: user.name, role: user.role, status: user.status, email: user.email, phone_number: user.phone_number };
    cookies().set('session', JSON.stringify(sessionPayload), { httpOnly: true, secure: process.env.NODE_ENV === 'production', maxAge: 60 * 60 * 24 * 7, path: '/' });
...
export async function sendVerificationOtpAction(prevState: SendOtpState, formData: FormData): Promise<SendOtpState> {
    ...
    if (!validatedPhone.success) {
        return { success: false, error: "A valid phone number is required.", userId, email };
    }
    ...
    try {
        ...
        if (devSettings.show_otp_in_admin) {
            await db.initialize();
            const newOtpEntry = { phone: phone, otp, expires: otpStore[phone].expires };
            db.tempOtps = [newOtpEntry, ...db.tempOtps].slice(0, 5);
...
export async function verifyPhoneAction(prevState: VerifyPhoneState, formData: FormData): Promise<VerifyPhoneState> {
    const phone = formData.get('phone') as string;
    ...
    if (!validatedFields.success) {
        return { success: false, error: validatedFields.error.flatten().fieldErrors.otp?.[0] || 'Invalid data.' };
    }
    ...
    const sessionPayload = { id: user.id, name: user.name, phone_number: user.phone_number, role: user.role, status: user.status, email: user.email };
    cookies().set('session', JSON.stringify(sessionPayload), { httpOnly: true, secure: process.env.NODE_ENV === 'production', maxAge: 60 * 60 * 24 * 7, path: '/' });
...
export async function requestPasswordReset(prevState: any, formData: FormData): Promise<{ success: boolean, message: string }> {
    ...
    if (user && user.role === 'admin') {
        ...
        db.passwordResetTokens.push({ userId: user.id, token, expiresAt: expiresAt.toISOString() });
...
export async function resetPasswordAction(prevState: any, formData: FormData): Promise<{ success: boolean; message: string, errors?: any }> {
    ...
    if (!tokenEntry || new Date(tokenEntry.expiresAt) < new Date()) {
        return { success: false, message: 'This password reset token is invalid or has expired.' };
    }
...
export async function initiateSignupAction(
  ...
  if (existingByPhone) {
    return { success: false, errors: { phone_number: ["An account with this phone number already exists."] } };
  }
  ...
  try {
    ...
    const otp = Math.floor(100000 + Math.random() * 900000).toString();
    otpStore[phone_number] = { otp, expires: Date.now() + 5 * 60 * 1000 };

    const devSettings = await getDeveloperSettings();
    if (devSettings.show_otp_in_admin) {
        const newOtpEntry = { phone: phone_number, otp, expires: otpStore[phone_number].expires };
        db.tempOtps = [newOtpEntry, ...db.tempOtps].slice(0, 5);
        await db.saveTempOtps();
    }
