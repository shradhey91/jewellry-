  ...
  return (
    <div className="space-y-4 rounded-lg border p-4">
      ...
      >
        ...
        <div
          className={size === "small" ? "space-y-4" : "md:col-span-2 space-y-4"}
        >
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              <Input
                ...
                value={defaultUrl}
                onChange={(e) =>
                  onImageChange(`imageUrl-${id}`, e.target.value)
                }
...
export function MobileAppearanceForm({
  ...
  const [heroSlides, setHeroSlides] = useState<HeroSliderItem[]>(
    content.heroSlider.items,
  );
  const [showIconText, setShowIconText] = useState(
    content.iconHighlights.showText ?? true,
  );
  const [videoEnabled, setVideoEnabled] = useState(
    content.hero.videoEnabled ?? true,
  );
  ...
  const handleImageChange = (fieldId: string, value: string) => {
    ...
    if (key === "hero" && id === "fallback") {
      const heroField =
        type === "imageUrl" ? "fallbackImageUrl" : "fallbackImageHint";
      setContent((prev) => ({
        ...prev,
        hero: { ...prev.hero, [heroField]: value },
      }));
    } else if (key === "banner" && id === "diamonds") {
      setContent((prev) => ({
        ...prev,
        imageBanner1: { ...prev.imageBanner1, [fieldToUpdate]: value },
      }));
    } else if (key === "banner" && id === "gifting") {
      setContent((prev) => ({
        ...prev,
        imageBanner2: { ...prev.imageBanner2, [fieldToUpdate]: value },
      }));
    } else if (key === "icon") {
      setContent((prev) => ({
        ...prev,
        iconHighlights: {
          ...prev.iconHighlights,
          items: prev.iconHighlights.items.map((i) =>
            i.id === id ? { ...i, [fieldToUpdate]: value } : i,
          ),
        },
      }));
      ...
      setContent((prev) => ({
        ...
        shopByCategory: {
          ...prev.shopByCategory,
          categories: prev.shopByCategory.categories.map((c) =>
            c.id === id ? { ...c, [fieldToUpdate]: value } : c,
          ),
        },
      }));
      ...
      setContent((prev) => ({
        ...
        testimonials: {
          ...
          items: prev.testimonials.items.map((t) =>
            t.id === id ? { ...t, [fieldToUpdate]: value } : t,
          ),
        },
      }));
    } else if (key === "journal") {
      setContent((prev) => ({
        ...
        journal: {
          ...prev.journal,
          entries: prev.journal.entries.map((e) =>
            e.id === id ? { ...e, [fieldToUpdate]: value } : e,
          ),
        },
      }));
    } else if (key === "heroSlider") {
      setHeroSlides((prev) =>
        prev.map((s) => (s.id === id ? { ...s, [fieldToUpdate]: value } : s)),
      );
    }
  };
  ...
  return (
    <form action={formAction}>
      ...
      <input
        type="hidden"
        name="imageUrl-hero-fallback"
        value={content.hero.fallbackImageUrl}
      />
      <input
        type="hidden"
        name="imageHint-hero-fallback"
        value={content.hero.fallbackImageHint || ""}
      />
      ...
      {content.iconHighlights.items.map((item) => (
        <React.Fragment key={item.id}>
          <input
            type="hidden"
            name={`imageUrl-icon-${item.id}`}
            value={item.imageUrl}
          />
          <input
            type="hidden"
            name={`imageHint-icon-${item.id}`}
      ...
      {content.shopByCategory.categories.map((cat) => (
        <React.Fragment key={cat.id}>
          <input
            type="hidden"
            name={`imageUrl-category-${cat.id}`}
            value={cat.imageUrl}
          />
          <input
            type="hidden"
            name={`imageHint-category-${cat.id}`}
            value={cat.imageHint}
          />
        </React.Fragment>
      ))}
      ...
      {content.testimonials.items.map((item) => (
        <React.Fragment key={item.id}>
          <input
            type="hidden"
            name={`imageUrl-testimonial-${item.id}`}
            value={item.imageUrl}
          />
          <input
            type="hidden"
            name={`imageHint-testimonial-${item.id}`}
            value={item.imageHint}
          />
        </React.Fragment>
      ))}
      ...
      {content.journal.entries.map((entry) => (
        <React.Fragment key={entry.id}>
          <input
            type="hidden"
            name={`imageUrl-journal-${entry.id}`}
            value={entry.imageUrl}
          />
          <input
            type="hidden"
            name={`imageHint-journal-${entry.id}`}
            value={entry.imageHint}
          />
        </React.Fragment>
      ))}
      ...
      <input
        type="hidden"
        name="imageUrl-banner-diamonds"
        value={content.imageBanner1.imageUrl}
      />
      <input
        type="hidden"
        name="imageHint-banner-diamonds"
        value={content.imageBanner1.imageHint}
      />

      <input
        type="hidden"
        name="imageUrl-banner-gifting"
        value={content.imageBanner2.imageUrl}
      />
      <input
        type="hidden"
        name="imageHint-banner-gifting"
        value={content.imageBanner2.imageHint}
      />
            ...
            <Card>
              <AccordionTrigger className="p-6 w-full">
                <CardHeader className="p-0 text-left">
                  ...
                  <CardDescription>
                    The main section at the top of your mobile homepage.
                  </CardDescription>
                </CardHeader>
              </AccordionTrigger>
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      defaultValue={content.hero.titleHighlight}
                    />
                  </div>
                  <div className="space-y-2">
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      id="hero-ctaLink"
                      name="hero-ctaLink"
                      defaultValue={content.hero.ctaLink}
                    />
                  </div>
                  <div className="flex items-center justify-between rounded-lg border p-4">
                    <div className="space-y-0.5">
                      <Label htmlFor="hero-videoEnabled" className="text-base">
                        Background Video
                      </Label>
                    ...
                    <Switch
                      id="hero-videoEnabled"
                      name="hero-videoEnabled"
                  ...
                  <ImageEditor
                    ...
                    defaultUrl={content.hero.fallbackImageUrl}
                    defaultHint={
                      content.hero.fallbackImageHint || "jewelry model"
                    }
                    hint="Displayed if the video fails to load or is disabled."
            ...
            <Card>
              <AccordionTrigger className="p-6 w-full">
                <CardHeader className="p-0 text-left">
                  ...
                  <CardDescription>
                    The icon strip below the hero section.
                  </CardDescription>
                </CardHeader>
              </AccordionTrigger>
              <AccordionContent className="px-6 pb-6">
                <div className="space-y-6">
                  <div className="flex items-center justify-between rounded-lg border p-4">
                    <div className="space-y-0.5">
                      <Label
                        htmlFor="iconHighlights-showText"
                        className="text-base"
                      >
                        Show Text Labels
                      </Label>
                    ...
                    <Switch
                      ...
                      onCheckedChange={setShowIconText}
                    />
                  </div>
            ...
            <Card>
              <AccordionTrigger className="p-6 w-full">
                <CardHeader className="p-0 text-left">
                  ...
                  <CardDescription>
                    An auto-playing carousel of promotional slides.
                  </CardDescription>
                </CardHeader>
              </AccordionTrigger>
              <AccordionContent className="px-6 pb-6">
                <div className="space-y-6">
                  {heroSlides.map((item, index) => (
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        <Label htmlFor={`heroSlider-${item.id}-title`}>
                          Title
                        </Label>
                        <Input
                          id={`heroSlider-${item.id}-title`}
                          name={`heroSlider-${item.id}-title`}
                          defaultValue={item.title}
                        />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor={`heroSlider-${item.id}-subtitle`}>
                          Subtitle
                        </Label>
                        <Textarea
                          id={`heroSlider-${item.id}-subtitle`}
                          name={`heroSlider-${item.id}-subtitle`}
                          defaultValue={item.subtitle}
                        />
                      </div>
                      <div className="grid md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          <Label htmlFor={`heroSlider-${item.id}-ctaText`}>
                            CTA Button Text
                          </Label>
                          <Input
                            id={`heroSlider-${item.id}-ctaText`}
                            name={`heroSlider-${item.id}-ctaText`}
                            defaultValue={item.ctaText}
                          />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor={`heroSlider-${item.id}-ctaLink`}>
                            CTA Button Link
                          </Label>
                          <Input
                            id={`heroSlider-${item.id}-ctaLink`}
                            name={`heroSlider-${item.id}-ctaLink`}
                            defaultValue={item.ctaLink}
                          />
                        </div>
                      </div>
                      <ImageEditor
                        id={`heroSlider-${item.id}`}
                        label="Background Image"
                        defaultUrl={item.imageUrl}
                        defaultHint={item.imageHint}
                        hint="Image for this slide."
                        onImageChange={handleImageChange}
                      />
                    </div>
                  ))}
            ...
            <Card>
              <AccordionTrigger className="p-6 w-full">
                <CardHeader className="p-0 text-left">
                  ...
                  <CardDescription>
                    Carousels for 'Newest In' and 'Best Sellers'.
                  </CardDescription>
                </CardHeader>
              </AccordionTrigger>
              <AccordionContent className="px-6 pb-6">
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="space-y-4 border p-4 rounded-lg">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        name="newest-categoryId"
                        defaultValue={
                          content.newestProducts.categoryId ?? "all"
                        }
                      >
                        <SelectTrigger>
                  ...
                  <div className="space-y-4 border p-4 rounded-lg">
                    ...
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        defaultValue={content.bestSellers.categoryId ?? "all"}
                      >
                        <SelectTrigger>
                          <SelectValue placeholder="Select a category" />
                        </SelectTrigger>
            ...
            <Card>
              <AccordionTrigger className="p-6 w-full">
                <CardHeader className="p-0 text-left">
                  ...
                  <CardDescription>
                    Full-width banners to highlight collections.
                  </CardDescription>
                </CardHeader>
              </AccordionTrigger>
              <AccordionContent className="px-6 pb-6">
                  ...
                  <div className="p-4 border rounded-lg space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        id="banner1-title"
                        name="banner1-title"
                        defaultValue={content.imageBanner1.title}
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        id="banner1-subtitle"
                        name="banner1-subtitle"
                        defaultValue={content.imageBanner1.subtitle}
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        id="banner1-ctaText"
                        name="banner1-ctaText"
                        defaultValue={content.imageBanner1.ctaText}
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        id="banner1-ctaLink"
                        name="banner1-ctaLink"
                        defaultValue={content.imageBanner1.ctaLink}
                      />
                    </div>
                    <ImageEditor
                      id="banner-diamonds"
                      label="Banner Image"
                      defaultUrl={content.imageBanner1.imageUrl}
                      defaultHint={content.imageBanner1.imageHint}
                      hint="Image for the first banner."
                      onImageChange={handleImageChange}
                    />
                  </div>
                  <div className="p-4 border rounded-lg space-y-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        id="banner2-title"
                        name="banner2-title"
                        defaultValue={content.imageBanner2.title}
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        id="banner2-subtitle"
                        name="banner2-subtitle"
                        defaultValue={content.imageBanner2.subtitle}
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        id="banner2-ctaText"
                        name="banner2-ctaText"
                        defaultValue={content.imageBanner2.ctaText}
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        id="banner2-ctaLink"
                        name="banner2-ctaLink"
                        defaultValue={content.imageBanner2.ctaLink}
                      />
                    </div>
                    <ImageEditor
                      id="banner-gifting"
                      label="Banner Image"
                      defaultUrl={content.imageBanner2.imageUrl}
                      defaultHint={content.imageBanner2.imageHint}
                      hint="Image for the second banner."
                      onImageChange={handleImageChange}
                    />
                  </div>
                </div>
            ...
            <Card>
              <AccordionTrigger className="p-6 w-full">
                <CardHeader className="p-0 text-left">
                  ...
                  <CardDescription>
                    Image grid for category navigation.
                  </CardDescription>
                </CardHeader>
              </AccordionTrigger>
              <AccordionContent className="px-6 pb-6">
                  ...
                  {content.shopByCategory.categories.map((category, index) => (
                    ...
                    >
                      ...
                      <ImageEditor
                        ...
                        defaultHint={category.imageHint}
                        hint="Image for this category."
                        onImageChange={handleImageChange}
                      />
                    </div>
                  ))}
            ...
            <Card>
              <AccordionTrigger className="p-6 w-full">
                <CardHeader className="p-0 text-left">
                  ...
                  <CardDescription>
                    Content for the customer testimonials section.
                  </CardDescription>
                </CardHeader>
              </AccordionTrigger>
              <AccordionContent className="px-6 pb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                  {content.testimonials.items.map((testimonial, index) => (
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        <Label htmlFor={`testimonial-${index}-text`}>
                          Text
                        </Label>
                        <Textarea
                          id={`testimonial-${index}-text`}
                          name={`testimonial-${index}-text`}
                      ...
                      <div className="space-y-2">
                        <Label htmlFor={`testimonial-${index}-location`}>
                          Location
                        </Label>
                        <Input
                          id={`testimonial-${index}-location`}
                          name={`testimonial-${index}-location`}
                      ...
                      <ImageEditor
                        ...
                        defaultUrl={testimonial.imageUrl}
                        defaultHint={testimonial.imageHint}
                        hint="Photo of the customer."
                        onImageChange={handleImageChange}
                      />
                    </div>
                  ))}
            ...
            <Card>
              <AccordionTrigger className="p-6 w-full">
                <CardHeader className="p-0 text-left">
                  ...
                  <CardDescription>
                    Content for the homepage blog preview section.
                  </CardDescription>
                </CardHeader>
              </AccordionTrigger>
              <AccordionContent className="px-6 pb-6">
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          ...
                          name={`journal-${index}-title`}
                          defaultValue={entry.title}
                        />
                      </div>
                      <div className="space-y-2">
                        <Label htmlFor={`journal-${index}-excerpt`}>
                          Excerpt
                        </Label>
                        <Textarea
                          id={`journal-${index}-excerpt`}
                          name={`journal-${index}-excerpt`}
                      ...
                      <div className="space-y-2">
                        ...
                        <Input
                          id={`journal-${index}-link`}
                          name={`journal-${index}-link`}
                      ...
                      <ImageEditor
                        id={`journal-${entry.id}`}
                        label="Article Image"
                        defaultUrl={entry.imageUrl}
                        defaultHint={entry.imageHint}
                        hint="Image for the journal entry."
                        onImageChange={handleImageChange}
                      />
                    </div>
                  ))}
