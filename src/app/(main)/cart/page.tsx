...
import { ShieldCheck, Truck, Wallet, Loader2, Tag, X, Sparkles, Heart, Gift, Star, ShoppingBag, Check } from 'lucide-react';
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 mt-6 md:mt-8">
            <div className="flex items-center gap-2 md:gap-3 bg-card px-3 md:px-5 py-3 md:py-4 rounded-xl shadow-md border border-border">
                ...
                <div className="min-w-0">
                    <p className="font-bold text-xs md:text-sm truncate">Purity Guaranteed</p>
                    <p className="text-xs text-muted-foreground truncate">on every online purchase</p>
                </div>
            </div>
            <div className="flex items-center gap-2 md:gap-3 bg-card px-3 md:px-5 py-3 md:py-4 rounded-xl shadow-md border border-border">
                ...
                <div className="min-w-0">
                    <p className="font-bold text-xs md:text-sm truncate">Secure Delivery</p>
                    <p className="text-xs text-muted-foreground truncate">by our trusted partners</p>
                </div>
            </div>
            <div className="flex items-center gap-2 md:gap-3 bg-card px-3 md:px-5 py-3 md:py-4 rounded-xl shadow-md border border-border sm:col-span-2 md:col-span-1">
                ...
                <div className="min-w-0">
                    <p className="font-bold text-xs md:text-sm truncate">Easy & Secure Payments</p>
                    <p className="text-xs text-muted-foreground truncate">backed by TATA</p>
                </div>
            </div>
        </div>
    )
...
export default function CartPage() {
  const { cartSubtotal, cartCount, cartTotal, discount, discountAmount, applyDiscount, removeDiscount } = useCart();
      ...
      try {
        ...
        const activeCoupons = allDiscounts
          ...
          .map(d => ({
            ...
            description: d.description || `${d.type === 'percentage' ? d.value + '%' : '₹' + d.value} OFF${d.min_purchase > 0 ? ' on orders above ₹' + d.min_purchase : ''}`
          }));
  ...
  return (
    <div className="bg-secondary/30 relative">
        ...
        <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
            <div className="absolute top-32 left-4 md:left-16 text-3xl md:text-5xl opacity-10 animate-bounce" style={{ animationDuration: '3s' }}>🛍️</div>
            <div className="absolute top-40 right-4 md:right-24 text-3xl md:text-4xl opacity-10 animate-bounce" style={{ animationDuration: '4s', animationDelay: '0.5s' }}>✨</div>
            <div className="absolute bottom-32 left-1/3 text-3xl md:text-4xl opacity-10 animate-bounce" style={{ animationDuration: '3.5s', animationDelay: '1s' }}>💝</div>
        </div>
            ...
            <>
                ...
                <div className="text-center mb-8 md:mb-10">
                    <div className="inline-flex items-center gap-1.5 md:gap-2 bg-gradient-to-r from-primary/10 via-primary/20 to-primary/10 px-4 md:px-6 py-2 md:py-3 rounded-full mb-4 md:mb-6 animate-pulse">
                        ...
                        <span className="text-sm md:text-base text-primary font-medium">Your Shopping Cart</span>
                    ...
                    <p className="text-base md:text-lg text-muted-foreground max-w-2xl mx-auto px-4">
                        {cartCount} {cartCount === 1 ? 'item' : 'items'} waiting for you. Let's make them yours! 🎉
                    </p>
                </div>
                ...
                <div className="flex justify-center mb-8 md:mb-12 overflow-x-auto">
                    <div className="flex items-center gap-2 md:gap-4 min-w-max">
                        <div className="flex items-center gap-1 md:gap-2">
                            ...
                            <span className="text-xs md:text-sm font-bold text-primary">Cart</span>
                        </div>
                        ...
                        <div className="flex items-center gap-1 md:gap-2">
                            ...
                            <span className="text-xs md:text-sm font-medium text-muted-foreground">Checkout</span>
                        </div>
                    ...
                    <div className="lg:sticky lg:top-28 space-y-4 md:space-y-6">
                                ...
                                <CardContent className="p-3 md:p-4">
                                    <div className="flex items-center gap-2 md:gap-4">
                                        <div className="relative w-24 h-16 md:w-28 md:h-20 shrink-0 rounded-lg overflow-hidden">
                                            <Image src="https://picsum.photos/seed/rewards-banner/200/150" layout="fill" objectFit="contain" alt="Rewards" data-ai-hint="rewards illustration" />
                                        </div>
                                        <div className="text-xs md:text-sm">
                                            <Link href="/auth/login" className="font-bold text-primary hover:underline flex items-center gap-1">
                                                <Heart className="w-3 h-3" />
                                                Login/Sign Up
                                            </Link>
                                            <p className="text-muted-foreground text-xs mt-0.5 md:mt-1">Unlock Exclusive Rewards</p>
                                        </div>
                                    </div>
                                </CardContent>
                            </Card>
                        ...
                        <Card className="border-2 border-primary/20 shadow-xl bg-gradient-to-br from-primary/5 via-card to-accent/10">
                            ...
                            <CardContent className="space-y-3 md:space-y-4">
                                    ...
                                    <AccordionItem value="coupon">
                                        <AccordionTrigger className="font-semibold text-xs md:text-sm py-2 md:py-3">
                                            <span className="flex items-center gap-1.5 md:gap-2">
                                                ...
                                                <span className="hidden xs:inline">Apply Coupon code / Promo Code</span>
                                            ...
                                            {discount ? (
                                                <div className="flex justify-between items-center rounded-lg border-2 border-green-500 bg-green-50 dark:bg-green-950/20 pl-2.5 md:pl-3 pr-1.5 md:pr-2 py-1.5 md:py-2">
                                                    <p className="text-xs md:text-sm font-bold text-green-700 dark:text-green-400 flex items-center gap-1.5 md:gap-2">
                                                        ...
                                                        <span className="hidden xs:inline">CODE APPLIED: {discount.code}</span>
                                                        <span className="xs:hidden">{discount.code}</span>
                                                    </p>
                                                    <Button variant="ghost" size="icon" className="h-7 w-7 md:h-8 md:w-8 hover:bg-green-100 dark:hover:bg-green-900" onClick={removeDiscount}>
                                                ...
                                                <div className="space-y-3 md:space-y-4">
                                                    <div className="flex space-x-2">
                                                        ...
                                                        />
                                                        <Button onClick={handleApplyCode} disabled={isLoading || !couponCode} className="bg-primary hover:bg-primary/90 text-sm px-3 md:px-4">
                                                            {isLoading ? <Loader2 className="h-3.5 w-3.5 md:h-4 md:w-4 animate-spin"/> : <span className="hidden xs:inline">Apply</span>}
                                                        </Button>
                                                    </div>
                                                                ...
                                                                {availableCoupons.map((coupon) => (
                                                                    ...
                                                                    >
                                                                        <div className="flex flex-col items-start gap-0.5 md:gap-1">
                                                                            <span className="font-bold text-xs md:text-sm text-primary">{coupon.code}</span>
                                                                            <span className="text-xs text-muted-foreground font-normal hidden md:inline">{coupon.description}</span>
                                                                        </div>
                                ...
                                <div className="space-y-2.5 md:space-y-3 pt-3 md:pt-4 text-sm">
                                    ...
                                    {discountAmount > 0 && (
                                        <div className="flex items-center justify-between text-green-600 dark:text-green-400">
                                            <span className="text-muted-foreground flex items-center gap-1">
                                                <Sparkles className="w-3 h-3" />
                                                <span className="hidden xs:inline">Product Discount</span>
                                    ...
                                    <div className="bg-gradient-to-r from-primary/10 to-accent/10 rounded-lg p-3 md:p-4 -mx-2">
                                        <div className="flex items-center justify-between">
                                            <span className="font-bold text-sm md:text-base">TOTAL (Incl. of all Taxes)</span>
                            ...
                            <CardFooter className="pt-3 md:pt-4">
                                <Button size="lg" className="w-full text-sm md:text-lg font-bold py-4 md:py-6 bg-gradient-to-r from-primary via-primary to-primary/80 hover:from-primary/90 hover:to-primary/70 shadow-xl hover:shadow-2xl hover:shadow-primary/30 transition-all duration-300 transform hover:-translate-y-1" asChild>
                                    <Link href="/checkout" className="flex items-center gap-1.5 md:gap-2">
