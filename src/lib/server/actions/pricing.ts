...
export async function toggleMetalActiveAction(
  metalId: string,
  isActive: boolean
  ...
  try {
    ...
    await db.initialize();
    const metal = db.metals.find(m => m.id === metalId);
    if (!metal) {
      return { success: false, message: 'Metal not found.' };
    }
    metal.is_active = isActive;
    metal.updated_at = new Date().toISOString();
    await db.saveMetals();
    revalidatePath('/admin/pricing');
    revalidatePath('/admin/history');
    return { success: true, message: `${metal.name} ${isActive ? 'activated' : 'deactivated'} successfully.` };
  } catch (error) {
    console.error('Error toggling metal active status:', error);
    return { success: false, message: 'Failed to update metal status.' };
  }
}
